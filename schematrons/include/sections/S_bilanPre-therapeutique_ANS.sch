<?xml version="1.0" encoding="UTF-8"?>


<!--  S_bilanPre-therapeutique_ANS
    
    Vérification de la conformité de la section FR-Bilan-pretherapeutique (1.2.250.1.213.1.1.2.42) créée par l'ANS
    
    Au 17/08/2017, cette section est utilisée par le volet AVC_AUNV
    
    Historique :
    17/08/2017 : LBE : Création
    20/10/2017 : NMA : Suppression du test sur la nomenclature utilisée pour coder la section
    30/01/2020 : NMA : Suppression du test sur le nombre de templateId
    
-->


<pattern xmlns="http://purl.oclc.org/dsdl/schematron" id="S_bilanPre-therapeutique_ANS">
    <title>Vérification de la conformité de la section FR-Bilan-pretherapeutique (1.2.250.1.213.1.1.2.42) créée par l'ANS</title>
    
    <rule context='*[cda:templateId/@root = "1.2.250.1.213.1.1.2.42"]'>

        <!-- Verifier que le templateId est utilisé pour une section -->
        <assert test="../cda:section"> 
            [S_bilanPre-therapeutique_ANS.sch] Erreur de Conformité CI-SIS : Cet élément ne peut être utilisé que comme section.
        </assert>
        
        <assert test="count(cda:id)=1">
            [S_bilanPre-therapeutique_ANS.sch]  Erreur de Conformité CI-SIS : La section doit contenir un id (cardinalité [1..1])
        </assert>
        
        <assert test="cda:text">
            [S_bilanPre-therapeutique_ANS.sch]  Erreur de Conformité CI-SIS : La section doit contenir un élément text
        </assert>        
        
        <!-- Vérifier le code de la section -->
        <assert test='cda:code[@code = "AVC-182"]'>
            [S_bilanPre-therapeutique_ANS.sch] Erreur de Conformité CI-SIS : Le code de la section Abdomen doit être AVC-182
        </assert>

        <!-- Vérification des entrées optionnelles -->
        <assert test="not(cda:entry) or cda:entry/cda:organizer/cda:templateId/@root='1.2.250.1.213.1.1.3.16'">
            [S_scoreGlasgow_ANS.sch]  Erreur de Conformité CI-SIS : Les entrées optionnelles autorisées sont : 
            Organizer Traitement initial AVC (1.2.250.1.213.1.1.3.16)
        </assert>

    </rule>

</pattern>
