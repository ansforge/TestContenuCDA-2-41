...
<xsl:template match="/">
      ...
      <svrl:schematron-output xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                              title="Vérification de la conformité au modèle AVC-EUNV_2.2"
                              schemaVersion="CI-SIS_AVC-EUNV_2.2.sch">
         ...
         <xsl:text/>
         <svrl:active-pattern>
            <xsl:attribute name="id">JDV_ComplicationNeurologique-CISIS</xsl:attribute>
            <svrl:text>Conformité d'un élément codé obligatoire par rapport à un jeu de valeurs du CI-SIS</svrl:text>
            <xsl:apply-templates/>
         </svrl:active-pattern>
         <xsl:apply-templates select="/" mode="M5"/>
         <svrl:active-pattern>
            <xsl:attribute name="id">Entete_AVC</xsl:attribute>
            <xsl:apply-templates/>
         </svrl:active-pattern>
   ...
   <!--SCHEMATRON PATTERNS-->
<svrl:text xmlns:svrl="http://purl.oclc.org/dsdl/svrl">Vérification de la conformité au modèle AVC-EUNV_2.2</svrl:text>

   <!--PATTERN JDV_ComplicationNeurologique-CISIS-->


	<!--RULE -->
<xsl:template match="/cda:ClinicalDocument/cda:component/cda:structuredBody/cda:component/cda:section/cda:entry/cda:observation[cda:code/@code='AVC-093']/cda:entryRelationship/cda:observation/cda:value"
                 priority="1000"
                 mode="M5">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                       context="/cda:ClinicalDocument/cda:component/cda:structuredBody/cda:component/cda:section/cda:entry/cda:observation[cda:code/@code='AVC-093']/cda:entryRelationship/cda:observation/cda:value"/>
...
<xsl:choose>
         ...
         <xsl:otherwise>
		    ...
		    <!--ASSERT -->
<xsl:choose>
         <xsl:when test="(             @nullFlavor or             (@xsi:type and not(@xsi:type = 'CD') and not(@xsi:type = 'CE')) or              (document($JDV_ComplicationNeurologique-CISIS)//svs:Concept[@code=$att_code and @codeSystem=$att_codeSystem])             )"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl">
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
            ...
            doit faire partie du jeu de valeurs <xsl:text/>
                  <xsl:value-of select="$JDV_ComplicationNeurologique-CISIS"/>
                  <xsl:text/>.
        </svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
...
<xsl:choose>
   ...
   <!--PATTERN variables-->
<xsl:variable name="JDV_ComplicationNeurologique-CISIS"
                 select="'../jeuxDeValeurs/JDV_ComplicationNeurologique-CISIS.xml'"/>

	  <!--RULE -->
<xsl:template match="cda:ClinicalDocument" priority="1000" mode="M8">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl" context="cda:ClinicalDocument"/>

		    <!--ASSERT -->
<xsl:choose>
         <xsl:when test="cda:templateId[@root='1.2.250.1.213.1.1.1.16']"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl">
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
